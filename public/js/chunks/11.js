...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    loginJWT: function loginJWT() {
      ...
      var self = this;
      _axios__WEBPACK_IMPORTED_MODULE_0__["default"].post('/api/auth/login/', {
        email: this.email,
        password: this.password
      }).then(function (response) {
        console.log(response.data.userData); //  this.$vs.loading.close()

        if (response.data.userData.name) {
          // Navigate User to homepage
          self.$router.push(self.$router.currentRoute.query.to || '/'); // Set accessToken

          localStorage.setItem("accessToken", response.data.access_token); // Update user details

          self.$store.commit('UPDATE_USER_INFO', response.data.userData, {
            root: true
          }); // Set bearer token in axios

          self.$store.commit("SET_BEARER", response.data.access_token); // resolve(response)
